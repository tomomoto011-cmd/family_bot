...
async def create_tables():
    async with pool.acquire() as conn:
        # 🔧 Миграция: безопасно добавляет колонки, если их нет
        ...
        # Создание таблиц
        for sql in [
            """CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, telegram_id BIGINT UNIQUE, username TEXT, role TEXT, family_id INT REFERENCES families(id), points INT DEFAULT 0, psycho_active BOOLEAN DEFAULT FALSE, created_at TIMESTAMP DEFAULT NOW())""",
...
async def create_user(tg, uname=None):
    async with pool.acquire() as c:
        await c.execute("INSERT INTO users(telegram_id,username) VALUES($1,$2) ON CONFLICT(telegram_id) DO UPDATE SET username=COALESCE($2,users.username)", tg, uname)

async def get_user(tg):
    async with pool.acquire() as c:
        return await c.fetchrow("SELECT * FROM users WHERE telegram_id=$1", tg)

async def set_role(tg, role):
    async with pool.acquire() as c:
        await c.execute("UPDATE users SET role=$1 WHERE telegram_id=$2", role, tg)

async def add_points(tg, amt):
    async with pool.acquire() as c:
        await c.execute("UPDATE users SET points=points+$1 WHERE telegram_id=$2", amt, tg)
...
async def set_psycho(tg, val):
    async with pool.acquire() as c:
        await c.execute("UPDATE users SET psycho_active=$1 WHERE telegram_id=$2", val, tg)
...
async def create_family(name, code=None):
    ...
    async with pool.acquire() as c:
        return await c.fetchrow("INSERT INTO families(name,code) VALUES($1,$2) RETURNING id,code", name, code)
...
async def get_family_members(fid):
    async with pool.acquire() as c:
        return await c.fetch("SELECT telegram_id, username, role, points FROM users WHERE family_id=$1 ORDER BY points DESC", fid)

async def get_pets(fid):
    async with pool.acquire() as c:
        return await c.fetch("SELECT * FROM pets WHERE family_id=$1 ORDER BY name", fid)

async def add_pet(fid, name):
    async with pool.acquire() as c:
        await c.execute("INSERT INTO pets(family_id,name) VALUES($1,$2)", fid, name)

async def get_family_stats(fid):
    async with pool.acquire() as c:
        return await c.fetch("SELECT u.telegram_id, u.username, u.points FROM users u WHERE u.family_id=$1 ORDER BY u.points DESC", fid)
...
async def assign_challenge(uid, cid):
    async with pool.acquire() as c:
        try:
            await c.execute("INSERT INTO user_challenges(user_id,challenge_id) VALUES($1,$2) ON CONFLICT(user_id,challenge_id,DATE(created_at)) DO NOTHING", uid, cid)
            return True
        except:
            return False
